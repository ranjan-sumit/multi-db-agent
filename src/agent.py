...
SCHEMA_PROMPT = """
You are a senior retail sales analytics agent. You answer questions by writing one safe read-only SQLite query.

Databases are already attached with these aliases and tables:
...
6. store.store_location
   ...
   - trade_area_type TEXT
   - avg_monthly_sales REAL
   - avg_monthly_units INTEGER
   - sales_per_sqft REAL
   - sales_band TEXT

7. calendar.fiscal_calendar
   - date_id INTEGER
   - fw_id TEXT
   - fiscal_year TEXT
   - fiscal_quarter TEXT
...
- BOBJ summary is aggregated current-year/prior-year sales.
- PBI detail is transaction/SKU-level detail.
- Product and pricing join to PBI detail by sku_id.
- Store hierarchy/location join to sales tables by store_id.
- Fiscal calendar joins to PBI detail by date_id and can also join by fw_id.
- Summary/detail exact reconciliation should aggregate PBI first, then join on fw_id and store_id.
- The dummy data may have sparse key coverage. If joins return very few rows, say that clearly.
- Use clear aliases.
- Add LIMIT 100 for detailed row lists.
...
def answer_question_rules(question: str) -> AgentAnswer:
    ...
    if "region" in q or "district" in q or "zone" in q:
        return sales_by_region()

    if "fiscal" in q or "quarter" in q or "month" in q:
        return sales_by_fiscal_quarter()

    if "price tier" in q or "pricing" in q:
        return sales_by_price_tier()

    if "square foot" in q or "sqft" in q or "trade area" in q:
        return sales_by_trade_area()
...
def sales_by_department() -> AgentAnswer:
    result = run_cross_database_query(
        """
        SELECT
            ph.department,
            SUM(d.net_sales) AS net_sales,
            SUM(d.net_units) AS net_units,
            COUNT(*) AS matched_detail_rows
        FROM pbi.sales_detail d
        JOIN product.product_hierarchy ph
            ON d.sku_id = ph.sku_id
        GROUP BY ph.department
        ORDER BY net_sales DESC
        LIMIT 25
        """
    )
    return AgentAnswer("Sales by department", _coverage_answer(result, "department", "SKU"), [result], result.data)


def sales_by_region() -> AgentAnswer:
    result = run_cross_database_query(
        """
        SELECT
            sh.region,
            ...
            COUNT(*) AS matched_detail_rows
        FROM pbi.sales_detail d
        JOIN store.store_hierarchy sh
            ON d.store_id = sh.store_id
        GROUP BY sh.region, sh.zone
        ORDER BY net_sales DESC
        LIMIT 25
        """
    )
    return AgentAnswer("Sales by region", _coverage_answer(result, "region/zone", "store"), [result], result.data)


def sales_by_fiscal_quarter() -> AgentAnswer:
    result = run_cross_database_query(
        """
        SELECT
            fc.fiscal_year,
            ...
            COUNT(*) AS matched_detail_rows
        FROM pbi.sales_detail d
        JOIN calendar.fiscal_calendar fc
            ON d.date_id = fc.date_id
        GROUP BY fc.fiscal_year, fc.fiscal_quarter
        ORDER BY fc.fiscal_year, fc.fiscal_quarter
        LIMIT 25
        """
    )
    return AgentAnswer("Sales by fiscal quarter", _coverage_answer(result, "fiscal quarter", "date"), [result], result.data)


def sales_by_price_tier() -> AgentAnswer:
    result = run_cross_database_query(
        """
        SELECT
            pp.price_tier,
            SUM(d.net_sales) AS net_sales,
            SUM(d.net_units) AS net_units,
            AVG(pp.retail_price) AS avg_retail_price,
            AVG(pp.actual_cost) AS avg_actual_cost,
            COUNT(*) AS matched_detail_rows
        FROM pbi.sales_detail d
        JOIN product.product_pricing pp
            ON d.sku_id = pp.sku_id
        GROUP BY pp.price_tier
        ORDER BY net_sales DESC
        LIMIT 25
        """
    )
    return AgentAnswer("Sales by price tier", _coverage_answer(result, "price tier", "SKU"), [result], result.data)


def sales_by_trade_area() -> AgentAnswer:
    result = run_cross_database_query(
        """
        SELECT
            sl.trade_area_type,
            ...
            SUM(d.net_sales) AS net_sales,
            SUM(d.net_units) AS net_units,
            AVG(sl.sales_per_sqft) AS avg_sales_per_sqft,
            COUNT(*) AS matched_detail_rows
        FROM pbi.sales_detail d
        JOIN store.store_location sl
            ON d.store_id = sl.store_id
        GROUP BY sl.trade_area_type, sl.sales_band
        ORDER BY net_sales DESC
        LIMIT 25
        """
    )
    return AgentAnswer("Sales by trade area", _coverage_answer(result, "trade area", "store"), [result], result.data)


def _coverage_answer(result: QueryResult, grain: str, key_name: str) -> str:
    if result.data.empty:
        return f"No rows matched for {grain}. Check whether sales {key_name} keys exist in the related master table."
    top = result.data.iloc[0]
    metric = top.get("net_sales", 0)
    matched = int(result.data["matched_detail_rows"].sum()) if "matched_detail_rows" in result.data else len(result.data)
    return (
        f"The query returned {len(result.data):,} {grain} row(s), based on {matched:,} matched sales detail row(s). "
        f"The top result has {_money(metric)} in net sales. Key coverage is sparse in this dummy data, so treat this as a join test rather than a complete business answer."
    )
